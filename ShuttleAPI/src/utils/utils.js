const {Setting,Wallet,Payment,Session,User,Pass,BookingAssign } = require('../models')
...
function generateOrderID(type){
    ...
    if(type){
        const name = type.charAt(0).toUpperCase() +''+type.charAt(1).toUpperCase()+''+type.charAt(2).toUpperCase();
        return name+rand;
    }
    return rand;
  }
...
const referCode = (length,phone) => {
    const result           = '';
    const characters       = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'+phone;
...
const getPassLists = async (minimum_fare, distance,tax,fee) => {
   ...
   let rdiscount = hasOneDigit(pass_discount) ? (pass_discount * 100)  : (pass_discount * 10);
        let totalsingleFare = "";
